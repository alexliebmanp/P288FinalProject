...
def build_url(year: int, jday: int, station: str, channel:str) -> str:
    ...
    """
    fname = f"{NETWORK}.{station}..{channel}.{year}.{jday:03d}"
    return f"{BASE_URL}/{year}/{NETWORK}/{station}/{channel}/{fname}"


def build_local_path(year: int, jday: int, station: str, channel:str) -> Path:
    """
    Local path:
        Magnetic/OV/<STATION>/OV.<STATION>..LF*.D.<year>.<jday>.mseed
    """
    fname = f"{NETWORK}.{station}..{channel}.{year}.{jday:03d}.mseed"
...
def main():
    ...
    while d <= END_DATE:
        ...
        for sta in STATIONS:
            ...
            for channel in CHANNEL_DIR:
                url = build_url(year, jday, sta, channel)
                out_path = build_local_path(year, jday, sta, channel)
    ...
    print(
        f"\nDone. Tried {n_total} files, "
        f"successfully present/downloaded {n_ok}."
    )
